...
var rotateFile = exports.rotateFile = winston.transports.rotateFile = function (options) {
    Transport.call(this, options);

    this.name = 'rotateFile';

    this.silent = options.silent || false;
    this.modulename = options.name;
    if (this.modulename)
        options.filename = options.filename.replace('.log', '.' + options.name + '.log');
...
rotateFile.prototype.open = function () {
    if (this.opening) {
        return false;
    }
...
// `_basename`).
rotateFile.prototype._createStream = function (dateString) {
    if (this.stream) {
        this.flush();
    }
    ...
    fs.exists(filename, function (exists) {
        self.stream = fs.createWriteStream(filename, {'flags': exists ? 'a' : 'w'});
...
rotateFile.prototype._createLink = function (filename) {
...
rotateFile.prototype._cleanOldFiles = function () {
    var whitelist = {};
    var date = new Date();
